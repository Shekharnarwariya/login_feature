...
public class ConnectionPool {
	...
	private Connection createConnection() {
		...
		try {
			...
			conn = DriverManager.getConnection(GlobalVars.DB_URL, GlobalVars.DB_USER, GlobalVars.DB_PASSWORD);
		} catch (Exception e) {
			logger.error("", e);
		}
		return conn;
	...
	public synchronized void initialize() {
		if (connections == null) {
			...
			try {
				Class.forName(GlobalVars.JDBC_DRIVER);
				int count = 0;
				while (count < GlobalVars.MAX_CONNECTIONS) {
					Connection c = DriverManager.getConnection(GlobalVars.DB_URL, GlobalVars.DB_USER,
							GlobalVars.DB_PASSWORD);
