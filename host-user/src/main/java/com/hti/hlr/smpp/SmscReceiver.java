...
/**
 * <code>Receiver</code> is class used for receiving PDUs from SMSC. It can be used two ways: it has methods for synchronous (blocking) receiving of PDUs and as it is derived from <code>ReceiverBase</code> whic on turn is derived from <code>ProcessingThread</code> class, it can also receive PDUs on
 * background and puts them into a queue.
 *
 ...
 */
/*
 * 13-07-01 ticp@logica.com start(), stop(), setQueueWaitTimeout() & getQueueWaitTimeout() made not synchronized; receive(long) & receive(PDU) made synchronized so the receiver no longer locks up 13-07-01 ticp@logica.com bug fixed in tryReceivePDU which caused that the PDUs were never removed from
 * the queue - now dequeue(expected) is now used instead of find(expected) 13-07-01 ticp@logica.com loads of debug lines corrected; some added 08-08-01 ticp@logica.com added support for Session's asynchronous processing capability 26-09-01 ticp@logica.com debug code categorized to groups 01-10-01
 * ticp@logica.com added function getThreadName for ProcessingThread thread name initialisation. 02-10-01 ticp@logica.com instead of importing full packages only the used classes are iported
 */
public class SmscReceiver extends ReceiverBase {
	...
	private Logger dlrlogger = LoggerFactory.getLogger("hlrDlrLogger");
	...
	/**
	 * The queue which holds the received PDUs. As the PDUs are received in asynchronnous manner, they are stored to a queue from which they can be get using method <code>receive</code>. PDUs are stored to the queue if and only if the <code>Receiver</code> is started as a separate thread using
	...
	/**
	 * This timeout specifies for how long will go the receiving into wait if the PDU (expected or any) isn't in the <code>pduQueue</code> yet. After that the queue is probed again (etc.) until receiving timeout expires or the PDU is received.
	 *
	...
	/**
	 * This object holds data received from connection which aren't complete PDU yet. If this situation occurs, it's likely that the data will be received the next time when another attempt to receive data from the connection occurs. Its used in <code>ReceiverBase</code>'s
	...
	/**
	 * If the receiving is asynchronous, <code>pduListener</code> must contain the callback object used for processing of PDUs received from the SMSC. <code>Receiver</code> after receiving a PDU passes the received PDU to apropriate member function of the processor.
	 *
	 * @see #asynchronous
	...
	public synchronized void setServerPDUEventListener(ServerPDUEventListener pduListener) {
		...
		if (asynchronous) {
			// System.out.println("Reciever sated to Asysnchronous");
	...
	public void stop() {
		...
		// System.out.println(smscId+" (((((( Receiver stoped )))))) \n");
		// debug.write(DRXTX, "Receiver stoped");
	}

	/**
	 * This method receives a PDU or returns PDU received on background, if there is any. It tries to receive a PDU for the specified timeout. If the receiver is asynchronous, then no attempt to receive a PDU and <code>null</code> is returned. The function calls are nested as follows:<br>
	 * <ul>
	...
	/**
	 * This method tries to receive one PDU from the connection. It is called in cycle from <code>tryReceivePDUWithTimeout</code> until timeout expires. <code>tryReceivePDUWithTimeout</code> is called either from <code>receiveAsync</code> as asynchronous receive on background or from
	 * <code>receive</code> as synchronous receive. It either gets pdu from the queue or tries to receive it from connection using <code>receivePDUFromConnection</code> depending on the value of the <code>receiver</code> flag. The method checks if the actualy received PUD is equal to
	...
	/**
	 * This method receives a PDU from connection and stores it into <code>pduQueue</code>. It's called from the <code>ReceiverBase</code>'s p<code>process</code> method which is called in loop from <code>ProcessingThread</code>'s <code>run</code> method.
	 * <p>
	 * If an exception occurs during receiving, depending on type of the exception this method either just reports the exception to debug & event objects or stops processing to indicate that it isn't able to process the exception. The function <code>setTermException</code> is then called with the
	 * caught exception.
	 *
	 * @see ReceiverBase#run()
	 */
	...
	protected void receiveAsync() {
		PDU pdu = null;
		// System.out.println(smscId + " <--- Receiving Asynchronous --->");
		try {
			...
			// System.out.println("<--- Recieving Socket Closing -->");
			// System.out.println("");
		}
		if (pdu != null) {
	...
	private void process(PDU pdu) {
		...
		if (pdu.getCommandId() == Data.DELIVER_SM) {
			dlrlogger.info(smscId + ":" + pdu.debugString());
		} else {
			pdulogger.info(smscId + ":" + pdu.debugString());
		}
		if (pduListener != null) {
	...
	/**
	 * Sends <code>GenericNack</code> PDU via transmitter if there is one. The <code>GenericNack</code> is sent in case that the message is corrupted or has unknown command id. If the sending of <code>GenericNack</code> fails, this method calls <code>stopProcessing</code> and thus stops the
	 * receiving thread.
	 *
	 * @param commandStatus
	 *            the error code
