...
/**
 * Exception <code>WrongSessionStateException</code> is thrown if a <code>Session</code>'s method which requires certain state of the session but the session is not in the state. Examples are submitting a message when the session is bound as a receiver or if method for receiving is called when the
 * session is bound as transmitter.
 *
 * @author Logica Mobile Networks SMPP Open Source Team
 * @version 1.0, 22 Nov 2001
 */
public class WrongSessionStateException extends SmppException {
	 ...
	 * 
	 * @param sessionType
	 *            the type of the session i.e. if it's session used in ESME or in MC
	 * @param expectedState
	 *            the state expected for the requested session operation
	 * @param currentState
	 *            the state the sesion was in when the state was checked
	 */
	...
	/**
	 * Construct the message carried by the excpetion according the details provided to the exception's constructor.
	 * 
	...
	public String getMessage() {
		if (hasSessionDetails) {
			...
			if (expectedState != Session.STATE_NOT_ALLOWED) {
				msg = "The operation is not allowed in the current " + typeDescription + " session state. "
						+ "Current state is " + getStateDescription(currentState) + " required state(s) is "
	...
	/**
	 * Returns the textual representation of the passed state. Can handle multiple states, e.g. for value "STATE_OPENED | STATE_CLOSED" returns string "closed, opened".
	 * 
	 * @param state
	 *            the state whose description has to be returned
	...
	/**
	 * Returns value of <code>descr</code> if the <code>state</code> and <code>testState</code> have at least one of the bits equaly set to 1. Uses <code>currentDescr</code> for checking if the returned string should be prefixed with comma.
	 * 
	 * @param state
	 *            the state value you probe
	 * @param testState
	 *            the state constant for which you provide <code>descr</code>
	 * @param currentDescr
	 *            if you are concatenating descriptions, pass the previously gained description here
	 * @param descr
	 *            the textual description of the state
	 * @return <code>descr</code> if the <code>state</code> contains <code>testState</code>, empty string otherwise
	 */
