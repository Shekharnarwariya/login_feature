 ...
 */
/*
 * 31-10-01 ticp@logica.com message length now stored and returned as short to accomodate the lengths > 127 (they're stored as negative values in byte vars) 31-10-01 ticp@logica.com SAR fields now
 * correctly return values > 127 19-11-01 ticp@logica.com added support for multibyte string encoding for short message
 */
public class SubmitSM extends Request {
	...
	private TLVOctets messagePayload = new TLVOctets(Data.OPT_PAR_MSG_PAYLOAD, Data.OPT_PAR_MSG_PAYLOAD_MIN,
			Data.OPT_PAR_MSG_PAYLOAD_MAX);
	private TLVByte privacyIndicator = new TLVByte(Data.OPT_PAR_PRIV_IND);
	private TLVOctets callbackNum = new TLVOctets(Data.OPT_PAR_CALLBACK_NUM, Data.OPT_PAR_CALLBACK_NUM_MIN,
			Data.OPT_PAR_CALLBACK_NUM_MAX); // 4-19
	private TLVByte callbackNumPresInd = new TLVByte(Data.OPT_PAR_CALLBACK_NUM_PRES_IND);
	private TLVOctets callbackNumAtag = new TLVOctets(Data.OPT_PAR_CALLBACK_NUM_ATAG,
			Data.OPT_PAR_CALLBACK_NUM_ATAG_MIN, Data.OPT_PAR_CALLBACK_NUM_ATAG_MAX); // 1-65
	private TLVOctets sourceSubaddress = new TLVOctets(Data.OPT_PAR_SRC_SUBADDR, Data.OPT_PAR_SRC_SUBADDR_MIN,
			Data.OPT_PAR_SRC_SUBADDR_MAX); // 2-23
	private TLVOctets destSubaddress = new TLVOctets(Data.OPT_PAR_DEST_SUBADDR, Data.OPT_PAR_DEST_SUBADDR_MIN,
			Data.OPT_PAR_DEST_SUBADDR_MAX);
	...
	public void setShortMessage(String value, String encoding)
		...
		setSmLength((short) shortMessage.getLength());
		// System.out.println("What is the value of Value and encoddinggg"+value);
		// System.out.println("What is the value of Value and encoddinggg"+encoding);
	}

	// Added by arshad ---------------------------------------
	public void setShortMessage(ByteBuffer data) throws WrongLengthOfStringException, UnsupportedEncodingException,
			PDUException, NotEnoughDataInByteBufferException, TerminatingZeroNotFoundException {
		shortMessage.setData(data);
		setSmLength((short) shortMessage.getLength());
	}
	...
	public String debugString() {
		...
		dbgs += "(sm: ";
		dbgs += "msg: ";
		if (getDataCoding() == (byte) 8 || getDataCoding() == (byte) 245) {
			try {
				dbgs += getShortMessage(Data.ENC_UTF16_BE);
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}
		} else {
			dbgs += getShortMessage();
		}
		dbgs += ") ";
		dbgs += " ";
