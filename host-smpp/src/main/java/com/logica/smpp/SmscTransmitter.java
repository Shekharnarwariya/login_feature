	...
	public void send(PDU pdu) throws ValueNotSetException, IOException {
		...
		try {
			...
			if (pdu.getCommandId() == Data.BIND_RECEIVER || pdu.getCommandId() == Data.BIND_TRANSCEIVER
				...
				pdulogger.info(smsc + " BindRequest: " + ((com.logica.smpp.pdu.BindRequest) pdu).getSystemId() + " seq:"
						+ pdu.getSequenceNumber());
			} else if (pdu.getCommandId() == Data.SUBMIT_SM) {
				submitlogger.info(smsc + " Esm: " + ((SubmitSM) pdu).getEsmClass() + " Dcs :"
						+ ((SubmitSM) pdu).getDataCoding() + " " + pdu.debugString());
			} else {
