...
import com.logica.smpp.pdu.HeaderIncompleteException;
import com.logica.smpp.pdu.MessageIncompleteException;
...
/**
 * Abstract base class for classes which can receive PDUs from connection. The receiving of PDUs can be be performed on background within a separate
 * thread using the <code>ProcessingThread</code> class.
 *
 * @author Logica Mobile Networks SMPP Open Source Team
 * @version 1.2, 01 Oct 2001
 * @see Receiver
 * @see OutbindReceiver
 * @see Connection
 * @see Transmitter
 ...
 */
/*
 * 13-07-01 ticp@logica.com start(), stop(), setReceiveTimeout(), getReceiveTimeout(), setTermException(), getTermException() made not synchronized;
 * receive(long) & receive(PDU) made synchronized so the receiver no longer locks up 13-07-01 ticp@logica.com the call to receiveAsync() in run() now
 * enclosed in try-finally and the status RCV_FINISHED is set in finally block so the finished status is now reported correctly even in case of
 * exception 13-07-01 ticp@logica.com some debug lines corrected; some added 08-08-01 ticp@logica.com added support for Session's asynchronous
 * processing capability 23-08-01 ticp@logica.com added async capability exhibited that if more than one pdu is read from connection at once (in one
 * connection.receive(), the additional ones aren't processed. this behaviour was corrected. 23-08-01 ticp@logica.com added yield() to run() to give
 * chance to other threads 26-09-01 ticp@logica.com debug code categorized to groups 01-10-01 ticp@logica.com now derived from new ProcessingThread
 * which implements the thread related issues; this change allows to focus only on the receiving related in the ReceiverBase and on the thread
 * management in the ProcessingThread; function covered by ProcessingThread removed
 */
public abstract class ReceiverBase extends ProcessingThread {
	 ...
	 */
	private Logger logger = LoggerFactory.getLogger(ReceiverBase.class);
	private long receiveTimeout = Data.RECEIVER_TIMEOUT;
	private byte messageIncompleteRetryCount = 0;

	/**
	 * Method repeatedly called from <code>process</code> method. It's expected that derived classes implement atomic receive of one PDU in this
	 * method using ReceiverBase's <code>tryReceivePDUWithTimeout</code> and <code>receivePDUFromConnection</code> methods.
	 *
	...
	/**
	 * This method should try to receive one PDU from the connection. It is called in cycle from <code>tryReceivePDUWithTimeout</code> until timeout
	 * expires. The method should check if the actualy received PDU is equal to the <code>expectedPDU</code>.
	 *
	 * @param connection
	 *            the connection from which the PDU should be received
	 * @param expectedPDU
	 *            the command id and sequence id of the received PDU should be equal to those of expectedPDU
	...
	/**
	 * This is an implementation of <code>ProcessingThread</code>'s <code>process</code> method, which is method called in loop from the
	...
	/**
	 * Calls <code>tryReceivePDUWithTimeout(Connection,PDU,long)</code> with timeout set by <code>setReceiveTimeout</code>.
	 *
	 * @param connection
	 *            the connection from which the PDU should be received
	 * @param expectedPDU
	...
	/**
	 * For specified time tries to receive a PDU from given connection by calling method <code>tryReceivePDU</code>. The method
	 * <code>tryReceivePDU</code> must be implemented in the derived class.
	 * <p>
	 * The timeout can be either value > 0, then it means for how many milliseconds will be repeatedly tried to receive a PDU. If the timeout is = 0
	 * then there is only one attempt to receive a PDU. If the timeout is equal to Data.RECEIVE_BLOCKING, then the this method tries receive a PDU
	...
	final protected PDU tryReceivePDUWithTimeout(Connection connection, PDU expectedPDU, long timeout)
			throws UnknownCommandIdException, TimeoutException, PDUException, IOException {
		logger.trace("receivePDU: Going to receive response.");
		...
		if (pdu != null) {
			logger.trace("Got pdu " + pdu.debugString());
		}
		return pdu;
	}

	/**
	 * Elementary method receiving data from connection, trying to create PDU from them and buffering data in case the PDU isn't still complete. It
	 * has timeout checking for incomplete messages: if the message isn't received completly for certain time and no new data are received for this
	 * time, then exception is thrown as this could indicate communication problem.
	 *
	 * @param connection
	 *            the connection to receive the data from
	...
	final protected PDU receivePDUFromConnection(Connection connection, Unprocessed unprocessed)
			throws UnknownCommandIdException, TimeoutException, PDUException, IOException {
		logger.trace("ReceiverBase.receivePDUFromConnection start");
		PDU pdu = null;
		ByteBuffer buffer;
		...
		try {
			...
			if (unprocessed.getHasUnprocessed()) {
				...
				logger.trace("have unprocessed " + unprocBuffer.length() + " bytes from previous try");
			...
			if (pdu == null) { // only if we didn't manage to get pdu from unproc
				...
				if (buffer.length() != 0) {
					...
					logger.trace("no data received this time.");
					...
					if ((unprocBuffer.length() > 0)
						...
						logger.trace("and it's been very long time.");
						unprocessed.reset();
			...
			logger.trace("There is _probably_ garbage in the unprocessed buffer - flushing unprocessed buffer now.");
			unprocessed.reset();
		}
		logger.trace("ReceiverBase.receivePDUFromConnection finished");
		return pdu;
	}

	/**
	 * Tries to create a PDU from the buffer provided. Returns the PDU if successfull or null if not or an exception if the PDU is incorrect.
	 */
	private final PDU tryGetUnprocessedPDU(Unprocessed unprocessed) throws UnknownCommandIdException, PDUException {
		logger.trace("trying to create pdu from unprocessed buffer");
		PDU pdu = null;
		...
		try {
			...
			logger.trace("incomplete message header, will wait for the rest.");
			...
			logger.trace("incomplete message, will wait for the rest.");
			unprocessed.setHasUnprocessed(false); // as it's incomplete - wait for new data
			unprocessed.setExpected(Data.PDU_HEADER_SIZE);
			messageIncompleteRetryCount++;
		} catch (UnknownCommandIdException e) {
			// message with invalid id was received, should send generic_nack
			logger.trace("unknown pdu, might remove from unprocessed buffer. CommandId=" + e.getCommandId());
		...
		/*
		 * paolo@bulksms.com: concerned that this is too broad, and will stop useful exceptions from being passed back up the call stack, so disabling
		 * for now: } catch (Exception e) { logger.trace(DRXTX, "Exception catched: " + e.toString()); StringWriter stringWriter = new StringWriter();
		 * PrintWriter printWriter = new PrintWriter(stringWriter); e.printStackTrace(printWriter); logger.trace(DRXTX, stringWriter.toString()); }
		 */
		if (pdu != null) {
			logger.trace("received complete pdu" + pdu.debugString());
			logger.trace("there is " + unprocBuffer.length() + " bytes left in unprocessed buffer");
		}
		// unprocessed.check();
		return pdu;
	}
	...
	/**
	 * Sets the timeout for receiving the complete message. If no data are received for time longer then this timeout and there is still not completly
	 * received PDU in the internal buffer, <code>TimeoutException</code> is thrown.
	 *
	 * @param timeout
	...
	/**
	 * Depending on value of <code>timeout</code> and on <code>startTime</code> returns if it's still possible to continue in receiving of message.
	 * <code>timeout</code> can indicate either timeout in milliseconds (if > 0), or that there has to be only one attempt to receive a message (if =
	 * 0) or that the the receiving shuld continue until a PDU is received (if = Data.RECEIVE_BLOCKING).
	 *
	 * @param startTime
	 *            when the receiving started
	 * @param timeout
	 *            timeout indication
