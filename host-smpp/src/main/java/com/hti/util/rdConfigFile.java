...
public class rdConfigFile {
	private Logger logger = LoggerFactory.getLogger(rdConfigFile.class);
		...
		try {
			...
			if (createNew) {
				try {
					readDefault = true;
					file.createNewFile();
					logger.info("File Created Successfully < " + file + " >");
					outputStream = new FileOutputStream(file);
					byte[] buff = "FLAG = 100\nRR = no\nUSER = no\nDB=no\nPrefix=no".getBytes();
					outputStream.write(buff);
					buff = null;
				} catch (IOException ioe) {
					logger.info("IOException:getValueSmsc(2)::" + ioe);
				} finally {
			...
			logger.info("IOException:getValueSmsc(3)::" + ioe);
		} catch (Exception ex) {
			logger.info("Exception:getValueSmsc(3)::" + ex);
		}
		// logger.info("1 >> " + arg_param + " = " + read_value);
		return read_value;
	}
	// ************************ End ********************************************
