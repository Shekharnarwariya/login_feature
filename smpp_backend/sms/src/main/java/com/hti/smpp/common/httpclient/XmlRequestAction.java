...
public class XmlRequestAction extends HttpServlet {
	private Logger logger = LoggerFactory.getLogger(XmlRequestAction.class);
	...
	private synchronized void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (status.equalsIgnoreCase(ResponseCode.NO_ERROR)) {
				...
				WriteXmlLogThread.logQueue.enqueue(request.getRemoteAddr() + " [" + batch_id + "] " + decodeUrl);
				...
				if (list != null && !list.isEmpty()) {
					...
					xmlDTO.setRequestFormat("XML");
					xmlDTO.setIpAddr(request.getRemoteAddr());
					int format = xmlDTO.getFormat();
					status = checkValidations(xmlDTO);
					logger.info("RequestBulk[" + format + "] --> " + xmlDTO);
					if (status.equalsIgnoreCase(ResponseCode.NO_ERROR)) {
						// -------- checking if invalid destination in list ------------
						int validReceipients = 0;
						if (format == 1) {
							Iterator<String> itr = xmlDTO.getReceipients().iterator();
							while (itr.hasNext()) {
							...
							Iterator<String[]> itr = xmlDTO.getCustomReceipients().iterator();
							while (itr.hasNext()) {
								String[] customarr = itr.next();
								String number = customarr[0];
								number = number.replaceAll("\\s+", ""); // Replace all the spaces in the String with
																		// empty character.
								number = number.substring(number.lastIndexOf("+") + 1); // Remove +
								try {
						...
						if (validReceipients == 0) {
							...
							if (xmlDTO.getScheduleTime() != null && xmlDTO.getScheduleTime().trim().length() == 12) {
								logger.info(batch_id + " Schedule Not allowed For XML requests");
								status = ResponseCode.INVALID_REQUEST;
							} else {
								xmlDTO.setWebid(batch_id);
								ApiRequestProcessor.procQueue.enqueue(xmlDTO);
							}
						}
					}
				} else {
					logger.info("Empty XML List Found.");
				}
			}
			StringBuffer xml_response = new StringBuffer();
			xml_response.append("<?xml version='1.0' encoding='UTF-8'?>");
			xml_response.append("<response>");
			xml_response.append("<status>" + status + "</status>");
			if (status.equalsIgnoreCase(ResponseCode.NO_ERROR)) {
				xml_response.append("<batch_id>" + batch_id + "</batch_id>");
			} else {
				xml_response.append("<batch_id></batch_id>");
			}
			xml_response.append("</response>");
			out.print(xml_response.toString());
			WriteXmlLogThread.logQueue.enqueue(batch_id + " Response: " + xml_response.toString());
		} catch (Exception e) {
	...
	private String checkValidations(BaseApiDTO xmlDTO) {
		logger.debug("Checking For Validations");
		...
		if (xmlDTO.getAccessKey() != null) {
			...
			if (!found) {
				logger.info(batch_id + " Invalid AccessKey: " + xmlDTO.getAccessKey());
				return ResponseCode.INVALID_LOGIN;
			}
		}
		String username = xmlDTO.getUsername();
		String password = xmlDTO.getPassword();
		String sender = xmlDTO.getSender();
		String text = xmlDTO.getText();
		int type = xmlDTO.getType();
		int format = xmlDTO.getFormat();
		if (username == null || password == null || sender == null) {
			...
			if (format == 1) {
				...
				if (xmlDTO.getReceipients().isEmpty()) {
					if (xmlDTO.getGroup().isEmpty()) {
						return ResponseCode.INVALID_REQUEST;
					} else {
						ContactService contactService = new ContactService();
						Map<String, GroupEntryDTO> group_list = contactService.listGroupNames(username);
						Map<String, GroupEntryDTO> valid_groups = new java.util.HashMap<String, GroupEntryDTO>();
						for (String group_name : xmlDTO.getGroup()) {
						...
						if (valid_groups.isEmpty()) {
							...
							for (GroupEntryDTO groupEntry : valid_groups.values()) {
								if (groupEntry.isGroupData()) {
									...
									if (xmlDTO.getLimit() > 0) {
										contact_list = contactService.listContact(groupEntry.getId(), 1,
												xmlDTO.getLimit());
									} else {
							...
							if (valid_numbers.isEmpty()) {
								...
								logger.info(batch_id + " Total Group Numbers To Proceed: " + valid_numbers.size());
								xmlDTO.getReceipients().addAll(valid_numbers);
							}
						}
					}
				}
			} else if (format == 2) {
				if (xmlDTO.getCustomReceipients().isEmpty()) {
					return ResponseCode.INVALID_REQUEST;
				}
			} else {
				return ResponseCode.INVALID_REQUEST;
		...
		if (format == 1 && text.length() == 0) {
			logger.error(batch_id + " Invalid Text: " + text);
			return ResponseCode.INVALID_TEXT;
		}
		if (type > 4) {
		...
		if (format == 1) {
			if (type == 0) {
				if (!Pattern.compile(
					...
					if (msg_length % 4 == 0) {
						try {
							String original = new Converters().UTF16(Converter.getUnicode(text.toCharArray()));
							...
							type = 1;
							System.out.println(batch_id + " auto detect type due to exception: " + type);
						}
					} else {
						type = 1;
						System.out.println(batch_id + " auto detect type: " + type);
				...
				if (type == 1 || type == 3) {
					...
					if (text.length() % 4 != 0) {
						logger.error(batch_id + " Invalid Unicode Message Length: " + text.length());
						return ResponseCode.INVALID_TEXT;
					}
				}
			}
		...
		try {
			...
			if (userEntry.getWebMasterEntry().isApiKeyOnly()) {
				if (xmlDTO.getAccessKey() != null && xmlDTO.getAccessKey().length() == 15) {
					if (!userEntry.getWebMasterEntry().getProvCode().equalsIgnoreCase(xmlDTO.getAccessKey())) {
						logger.info(batch_id + " " + username + " Mismatched AccessKey: " + xmlDTO.getAccessKey());
						return ResponseCode.INVALID_LOGIN;
					}
				} else {
					logger.info(batch_id + " " + username + " Invalid AccessKey: " + xmlDTO.getAccessKey());
					return ResponseCode.INVALID_LOGIN;
				}
			}
			if (!userEntry.getUserEntry().getPassword().equalsIgnoreCase(password)) {
				logger.info(batch_id + " " + username + " Invalid Password: " + password);
				return ResponseCode.INVALID_LOGIN;
			}
			if (!userEntry.getWebMasterEntry().isApiAccess()) {
				logger.info(batch_id + " " + username + " ApiAccess Not Allowed");
				return ResponseCode.ACCESS_DENIED;
			}
			if ((userEntry.getWebMasterEntry().getSenderId() != null
				...
				if (!userEntry.getWebMasterEntry().getSenderId().toLowerCase().contains(sender.toLowerCase())) {
					logger.error(batch_id + " SenderId Not Allowed: " + sender);
					return ResponseCode.INVALID_SENDER;
				}
			}
			...
			if (userEntry.getUserEntry().getAccessIp() != null && userEntry.getUserEntry().getAccessIp().length() > 0) {
				...
				if (xmlDTO.getIpAddr().equalsIgnoreCase("0:0:0:0:0:0:0:1")
						|| xmlDTO.getIpAddr().equalsIgnoreCase("127.0.0.1")) {
					matched = true;
				} else {
					...
					while (st.hasMoreTokens()) {
						...
						if (allowedip.indexOf("/") > 0) {
							if (isInRange(allowedip, xmlDTO.getIpAddr())) {
								matched = true;
								break;
							}
						} else {
							if (xmlDTO.getIpAddr().equalsIgnoreCase(allowedip)) {
								matched = true;
								break;
							}
						}
				...
				if (!matched) {
					logger.error(username + " Invalid XML Access IPAddress: " + xmlDTO.getIpAddr());
					return ResponseCode.ACCESS_DENIED;
				}
			} else {
				if (userEntry.getUserEntry().getAccessCountry() != null
					...
					if (xmlDTO.getIpAddr().equalsIgnoreCase("0:0:0:0:0:0:0:1")
						...
						String country = new LoginService().getCountryname(xmlDTO.getIpAddr());
						if (country != null) {
							...
							logger.info(username + " Country [" + xmlDTO.getIpAddr() + "] not found in database.");
						}
					}
					if (!matched) {
						logger.error(
								batch_id + " " + username + " Invalid XML Access IPAddress: " + xmlDTO.getIpAddr());
						return ResponseCode.ACCESS_DENIED;
					}
				}
			}
			...
			logger.error(batch_id, ex.fillInStackTrace());
		}
		// --------- End if Account blocked --------
