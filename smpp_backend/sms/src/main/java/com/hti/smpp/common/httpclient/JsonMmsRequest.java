...
public class JsonMmsRequest extends HttpServlet {
	...
	private BaseApiDTO parseRequest(String uri) throws JSONException, InvalidFormatException {
		...
		if (parse.has("campaign")) {
			...
			if (((campaign.has("username") && campaign.has("password")) || campaign.has("accesskey"))
					&& campaign.has("sender")) {
				json = new BaseApiDTO();
				...
				if (campaign.has("gsm")) {
					...
					if (receipients.length() == 0) {
						...
						while (itr.hasNext()) {
							...
							number = number.replaceAll("\\s+", ""); // Replace all the spaces in the String with empty
																	// character.
					...
					if (group.length() == 0) {
						...
						Map<String, GroupEntryDTO> group_list = contactService.listGroupNames(json.getUsername());
						...
						if (valid_groups.isEmpty()) {
							...
							for (GroupEntryDTO groupEntry : valid_groups.values()) {
								if (groupEntry.isGroupData()) {
									...
									if (limit > 0) {
										contact_list = contactService.listGroupData(groupEntry.getId(), 1, limit);
									} else {
				...
				if (campaign.has("mms_type")) {
					String mmsType = campaign.getString("mms_type");
					mmsType = mmsType.toLowerCase();
					if (mmsType.equals("image") || mmsType.equals("audio") || mmsType.equals("video")
							|| mmsType.equals("vcard")) {
						json.setMmsType(mmsType);
					} else {
						logger.error(batch_id + " Invalid mms_type: " + mmsType);
						throw new InvalidFormatException(ResponseCode.INVALID_TYPE);
					}
				} else {
					logger.error(batch_id + " Parameter missing: mms_type");
					throw new InvalidFormatException(ResponseCode.INVALID_TYPE);
				}
				if (campaign.has("url")) {
					text = campaign.getString("url");
					int type = 1;
					if (text.length() == 0) {
						logger.error(batch_id + " Invalid Url: " + text);
						...
						if (!Pattern.compile(
								...
								.matcher(text).find()) {
							type = 4;
							System.out.println(batch_id + " auto detect content type: " + type);
						}
					}
					if (type == 4 && text.length() > 70) {
						logger.error(batch_id + " Invalid Url length: " + text.length());
						throw new InvalidFormatException(ResponseCode.INVALID_TEXT);
					} else if (type == 1 && text.length() > 160) {
						logger.error(batch_id + " Invalid Url length: " + text.length());
						throw new InvalidFormatException(ResponseCode.INVALID_TEXT);
					}
					json.setText(text);
					json.setType(type);
					if (campaign.has("caption")) {
						json.setCaption(campaign.getString("caption"));
					}
				} else {
					logger.error(batch_id + " Parameter missing: url");
					throw new InvalidFormatException(ResponseCode.INVALID_TEXT);
				}
			} else {
