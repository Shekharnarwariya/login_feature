...
public interface KeywordEntryRepository extends JpaRepository<KeywordEntry, Integer> {
	...
	@Query("SELECT k FROM KeywordEntry k WHERE (k.createdOn BETWEEN :start AND :end) AND k.type = :type")
	Page<KeywordEntry> searchByDate(@Param("start") String start, @Param("end") String end,@Param("type") String type, Pageable p);
	...
	@Query("SELECT k FROM KeywordEntry k WHERE (k.createdOn BETWEEN :start AND :end) AND k.type = :type AND k.userId IN :users")
	Page<KeywordEntry> searchByDateAndFindByUserIdIn(@Param("start") String start, @Param("end") String end, @Param("type") String type, @Param("users") Integer[] users,Pageable p);
}
